...
function createHarness() {
    ...
    const sandboxWindow = {
        ...
        PracticeRecordAPI: {
            ...
            async mergeRecords(nextRecords, options = {}) {
                const incoming = Array.isArray(nextRecords)
                    ? nextRecords.map((record, index) => this.normalizeRecord(record, { index })).filter(Boolean)
                    : [];
                const mergeMode = options.mergeMode || 'merge';
                if (mergeMode === 'replace') {
                    await this.replace(incoming, { updateStats: options.updateStats !== false });
                    return {
                ...
                await this.replace(merged, { updateStats: options.updateStats !== false });
                return {
                    importedCount,
                    updatedCount,
                    skippedCount: 0,
            ...
            async restoreRecords(nextRecords, options = {}) {
                await this.replace(Array.isArray(nextRecords) ? nextRecords : [], { updateStats: false });
                if (options && options.stats && typeof options.stats === 'object') {
                    await this.resetStats(options.stats);
                } else if (options.updateStats !== false) {
                    await this.recalculateStats();
                }
                return {
                    restoredCount: records.length,
                    statsRestored: Boolean(options && options.stats && typeof options.stats === 'object')
                };
            },
            prepareStats(stats) {
                const source = stats && typeof stats === 'object' ? clone(stats) : {};
                return {
                    totalPractices: 0,
                    totalTimeSpent: 0,
                    averageScore: 0,
                    ...
                    achievements: [],
                    ...source,
                    categoryStats: source.categoryStats && typeof source.categoryStats === 'object' ? clone(source.categoryStats) : {},
                    questionTypeStats: source.questionTypeStats && typeof source.questionTypeStats === 'object' ? clone(source.questionTypeStats) : {},
                    practiceDays: Array.isArray(source.practiceDays) ? source.practiceDays.slice() : [],
                    achievements: Array.isArray(source.achievements) ? source.achievements.slice() : []
                };
            },
            normalizeRecord(record, options = {}) {
                const core = sandboxWindow.PracticeCore;
                if (!core || !core.contracts || typeof core.contracts.standardizeRecord !== 'function') {
                    throw new Error('PracticeCore.contracts.standardizeRecord should be ready');
                }
                return core.contracts.standardizeRecord(record, options);
            },
            async readStats() {
