...
## 【HistoryPage 设计约束（必须写死，避免歧义与垃圾分支）】
- **筛选契约**：前端统一构造
  - `date_range` → `{ start_date, end_date }`（ISO 字符串）
  - `score_range` → `{ min_score, max_score }`（0.5 步长）
  - `search` → 后端 LIKE（题目标题 + 内容）
- **导出语义**：默认导出“当前筛选结果（全量）”，不是“当前页”
- **批量删除语义**：只删除 `selectedIds`，与筛选结果全量删除严格区分
- **确认流程**：批量删除/清空必须二次确认，避免误删
