(function (global) {
    ...
    const READING_COACH_ATTEMPT_PLAN = Object.freeze([
    ...
    function normalizeReadingCoachRequestPayload(data) {
        ...
        return {
            ...
            locale: typeof payload.locale === 'string' && payload.locale.trim().toLowerCase() === 'en' ? 'en' : 'zh',
            surface: typeof payload.surface === 'string' ? payload.surface.trim() : '',
            action: typeof payload.action === 'string' ? payload.action.trim() : 'chat',
            promptKind: typeof payload.promptKind === 'string' ? payload.promptKind.trim() : '',
            selectedText: typeof payload.selectedText === 'string'
                ? payload.selectedText.trim()
                : (typeof payload.selectedContext?.text === 'string' ? payload.selectedContext.text.trim() : ''),
            selectedContext: isRecordObject(payload.selectedContext) ? payload.selectedContext : null,
            focusQuestionNumbers: Array.isArray(payload.focusQuestionNumbers)
                ? payload.focusQuestionNumbers.map((item) => String(item || '').trim()).filter(Boolean)
                : [],
            history: Array.isArray(payload.history) ? payload.history : [],
            attemptContext: isRecordObject(payload.attemptContext)
                ? payload.attemptContext
                : (isRecordObject(payload.practiceContext) ? payload.practiceContext : {})
        };
    }
    ...
    function normalizeReadingCoachBridgeResultPayload(payload) {
        if (!payload || payload.success === false) {
            throw createReadingAnalysisBridgeError({
                code: payload?.error?.code || 'reading_coach_ipc_failed',
                message: payload?.error?.message || payload?.message || '阅读教练请求失败'
            }, 'reading_coach_ipc_failed', '阅读教练请求失败', {
                stage: 'ipc_response',
                success: false
            });
        }
        if (!isRecordObject(payload.data)) {
            throw createReadingAnalysisBridgeError({
                ...
                message: '阅读教练返回格式无效'
            }, 'invalid_response_format', '阅读教练返回格式无效', {
                stage: 'ipc_response',
                success: true,
                hasData: false
            });
    ...
    const mixin = {
        ...
        async openExam(examId, options = {}) {
            ...
            try {
                ...
                if (readingLaunch && readingLaunch.mode === 'unified_html' && !guardOptions.target) {
                    guardOptions.target = 'inline';
                }
                ...
                examUrl = await this._appendRuntimeContextToExamUrl(examUrl);
                ...
                this.setupExamWindowManagement(examWindow, examId, exam, guardOptions);
        ...
        openExamWindow(examUrl, exam, options = {}) {
            ...
            if (options.target === 'inline') {
                try {
                    const inlineWindow = this._openInlineExamWindow(finalUrl, exam, options);
                    if (inlineWindow) {
                        return inlineWindow;
                    }
                } catch (error) {
                    console.warn('[App] 内嵌练习页创建失败，回退到弹窗模式:', error);
                }
            }
        ...
        _ensureInlinePracticeHost() {
            if (this.inlinePracticeHost && this.inlinePracticeHost.container && document.body && document.body.contains(this.inlinePracticeHost.container)) {
                return this.inlinePracticeHost;
            }
            ...
            container.id = 'inline-practice-host';
            ...
            frame.id = 'inline-practice-frame';
            frame.setAttribute('title', 'IELTS 练习');
            frame.style.cssText = 'border: 0;background: #ffffff;width: 100%;height: 100%;display: block;';

            shell.appendChild(frame);
            container.appendChild(shell);
            document.body.appendChild(container);

            this.inlinePracticeHost = {
                container,
                shell,
                frame,
                activeExamId: null
        ...
        _teardownInlinePracticeHost() {
            const host = this.inlinePracticeHost;
            if (!host) {
                return;
            }
            try {
                ...
                // ignore iframe reset errors
            }
            try {
                if (host.container && host.container.parentNode) {
                    host.container.parentNode.removeChild(host.container);
                }
            } catch (_) {
                // ignore DOM remove errors
            }
            this.inlinePracticeHost = null;
        },

        _openInlineExamWindow(examUrl, exam, options = {}) {
            const host = this._ensureInlinePracticeHost();
            if (!host || !host.frame) {
                return null;
            }
            host.activeExamId = exam && exam.id ? exam.id : null;
            host.container.style.display = 'flex';
            options.inlineMode = true;
            options.inlineFrame = host.frame;
            try {
                host.frame.src = examUrl;
            } catch (error) {
                console.warn('[InlinePractice] 无法设置 iframe src:', error);
                return null;
            }
            return host.frame.contentWindow || null;
        },
        ...
        async _appendRuntimeContextToExamUrl(rawUrl) {
            if (!rawUrl) {
                return rawUrl;
            }
            try {
                ...
                if (window && window.location && /^app:\/\/app(?:\/|$)/.test(String(window.location.href || ''))) {
                    safeSet('legacyHomeUrl', 'app://app/index.html');
                }
                ...
                safeSet('runtime', 'electron');

                try {
                    const response = await window.electronAPI.getLocalApiInfo();
                    const baseUrl = response && response.data && typeof response.data.baseUrl === 'string'
                        ? response.data.baseUrl.trim()
                        : '';
                    safeSet('localApiBaseUrl', baseUrl);
                } catch (error) {
                    console.warn('[App] 读取本地分析服务信息失败:', error);
                }

                return parsed.toString();
            } catch (error) {
                console.warn('[App] 添加练习页运行时上下文失败:', error);
                return rawUrl;
            }
        },
        ...
        setupExamWindowManagement(examWindow, examId, exam = null, options = {}) {
            ...
            const isInlineMode = options && (options.inlineMode === true || options.target === 'inline');
            ...
            this.examWindows.set(examId, {
                ...
                inlineFrame: options && options.inlineFrame ? options.inlineFrame : null,
            ...
            if (isInlineMode && options.inlineFrame && typeof options.inlineFrame.addEventListener === 'function') {
                try {
                    options.inlineFrame.addEventListener('load', emitInitEnvelope, { once: true });
                } catch (error) {
                    console.warn('[App] 监听内嵌练习页 load 事件失败:', error);
                    emitInitEnvelope();
                }
            } else if (!isFileProtocol) {
                try {
                    examWindow.addEventListener('load', emitInitEnvelope, { once: true });
                } catch (error) {
        ...
        setupExamWindowCommunication(examWindow, examId, exam = null, options = {}) {
            ...
            const messageHandler = async (event) => {
                ...
                const bridgeRequestId = data && data.requestId != null ? String(data.requestId).trim() : '';
                const replyReadingBridgeNack = (code, message, meta = {}) => {
                    const isCoachRequest = type === 'REQUEST_READING_COACH';
                    if (!isCoachRequest || !bridgeRequestId || !sourceWindow || typeof sourceWindow.postMessage !== 'function') {
                        return;
                    }
                    try {
                        sourceWindow.postMessage({
                            type: 'READING_COACH_RESULT',
                            data: {
                ...
                if (event.origin && event.origin !== 'null') {
                    ...
                    if (allowedOrigin && event.origin !== allowedOrigin) {
                        replyReadingBridgeNack('analysis_bridge_origin_mismatch', '父页拒绝了跨域分析请求', {
                ...
                if (src && !allowedSources.has(src)) {
                    replyReadingBridgeNack('analysis_bridge_source_not_allowed', '父页拒绝了未知来源的分析请求', {
                        sourceTag: src
                    });
                    return; // 非预期来源的消息忽略
                }
                ...
                if (!sourceMatched && !allowSuiteSourceFallback) {
                    replyReadingBridgeNack('analysis_bridge_source_mismatch', '父页未识别当前分析请求来源窗口', {
                        payloadExamId,
                        expectedExamId
                    });
                    return;
                ...
                if (payloadSessionId) {
                    if (expectedSessionId && payloadSessionId !== expectedSessionId) {
                        ...
                        if (!allowSuiteSessionMismatch) {
                            replyReadingBridgeNack('analysis_bridge_session_mismatch', '父页拒绝了会话不匹配的分析请求', {
                                payloadSessionId,
                                expectedSessionId
                            });
                            return;
                ...
                if (payloadExamId && payloadExamId !== expectedExamId) {
                    ...
                    if (!allowedLegacy) {
                        replyReadingBridgeNack('analysis_bridge_exam_mismatch', '父页拒绝了题目编号不匹配的分析请求', {
                            payloadExamId,
                            expectedExamId
                        });
                        return;
                ...
                switch (type) {
                    ...
                    case 'PROGRESS_UPDATE':
                        ...
                        break;
                    case 'PRACTICE_EXIT':
                        await this.handlePracticeExit(examId, data, sourceWindow || expectedWindow);
                        break;
                    case 'REQUEST_READING_COACH':
                        await this.handleReadingCoachRequest(examId, data, sourceWindow || expectedWindow);
                        break;
        ...
        async handlePracticeExit(examId, data, sourceWindow = null) {
            ...
            const inSuiteFlow = hasMappedSuiteExam || hasActiveSuiteSession || Boolean(payloadSuiteSessionId);

            if (inSuiteFlow && typeof this._abortSuiteSession === 'function' && this.currentSuiteSession) {
                window.showMessage && window.showMessage('已退出套题练习。', 'info');
                await this._abortSuiteSession(this.currentSuiteSession, { reason: 'practice_exit' });
                return;
            }

            this.updateExamStatus(examId, 'interrupted');
            await this.cleanupExamSession(examId, {
                forceCloseInlineHost: true,
                sourceWindow
            });
        },

        async handleReadingCoachRequest(examId, data, sourceWindow = null) {
            if (!sourceWindow || typeof sourceWindow.postMessage !== 'function') {
                return;
            }
            const normalizedRequest = normalizeReadingCoachRequestPayload(data);
            ...
            if (!requestId) {
                respondError('analysis_bridge_invalid_request', '父页收到的阅读教练请求缺少 requestId', {
                    stage: 'validate',
                    status: 'rejected'
                });
                return;
            ...
            if (!normalizedRequest.query) {
                respondError('analysis_bridge_invalid_payload', '父页收到的阅读教练请求缺少 query', {
                    stage: 'validate',
                    status: 'rejected'
                });
                return;
            ...
            const runWithTimeout = (taskFactory, timeoutMs) => new Promise((resolve, reject) => {
                ...
                const timer = setTimeout(() => {
                    ...
                    reject(createReadingAnalysisBridgeError({
                        ...
                        message: `阅读教练服务响应超时（${Math.ceil(normalizedTimeout / 1000)}s）`
                    }, 'analysis_bridge_upstream_timeout', `阅读教练服务响应超时（${Math.ceil(normalizedTimeout / 1000)}s）`, {
            ...
            for (let attemptIndex = 0; attemptIndex < READING_COACH_ATTEMPT_PLAN.length; attemptIndex += 1) {
                ...
                try {
                    ...
                    if (!isRetryableReadingAnalysisBridgeError(lastError) || attemptIndex >= READING_COACH_ATTEMPT_PLAN.length - 1) {
                        break;
                    }
            ...
            const finalError = lastError || createReadingAnalysisBridgeError(null, 'analysis_bridge_failed', '阅读教练桥调用失败', {
                stage: 'finalize_attempts'
            });
            respondError(finalError.code || 'analysis_bridge_failed', finalError.message || '阅读教练桥调用失败', {
                attempts: READING_COACH_ATTEMPT_PLAN.length,
                elapsedMs: Date.now() - startedAt,
        ...
        async cleanupExamSession(examId, options = {}) {
            ...
            const shouldCloseInlineHost = Boolean(
                options.forceCloseInlineHost
                || (windowInfo && windowInfo.inlineMode && this.inlinePracticeHost && this.inlinePracticeHost.activeExamId === examId)
            );
            if (shouldCloseInlineHost) {
                this._teardownInlinePracticeHost();
            }
        },
