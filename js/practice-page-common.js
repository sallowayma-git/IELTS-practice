/**
 * 通用练习页面脚本
 * 包含了基础UI交互（计时器、设置、笔记、高亮、面板调整）
 * 和练习页面增强器（跨窗口通信、答案收集、正确答案提取、分数计算、数据提交）
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let draggedElement = null; // For drag and drop questions

    // Correct answers will be populated by the specific exam page or extracted
    // by the CorrectAnswerExtractor.
    ...
    // --- Event Listeners Setup (General UI) ---
    startTimer();
    ...
    if (divider && leftPane) { // Only apply if the elements exist (i.e., not a pure listening page)
    ...
    if (!window.practicePageEnhancer) {
        ...
        window.practicePageEnhancer = {
            ...
            parentWindow: null,
            answers: {},
            correctAnswers: correctAnswers, // Use the global correctAnswers object
            interactions: [],
            startTime: Date.now(),
            isInitialized: false,
            ...
            detectPageType: function () {
                ...
                if (title.includes('P4') || url.includes('P4')) return 'P4'; // Added P4 for listening
                const pathParts = url.split('/');
                for (let part of pathParts) {
                    if (part.match(/^P[1234]$/i)) { // Added P4 for listening
                ...
                if (window.CorrectAnswerExtractor) {
                    try {
                        ...
                        if (extractedAnswers && Object.keys(extractedAnswers).length > 0) {
                            // Merge extracted answers with any pre-defined correctAnswers
                            Object.assign(this.correctAnswers, extractedAnswers);
