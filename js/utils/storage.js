...
const createStorageInternalAccessOptions = (options = {}) => {
...
const hasStorageInternalAccess = (options = {}) => {
...
class StorageManager {
    ...
    async writePersistentValue(key, value, options = {}) {
        if (this.isProtectedDataKey(key) && !hasStorageInternalAccess(options)) {
    ...
    async clearPersistentStorage(options = {}) {
        if (!hasStorageInternalAccess(options)) {
    ...
    async set(key, value, options = {}) {
        const { skipReady = false } = options;
        const protectedPublicAccess = this.isProtectedDataKey(key) && !hasStorageInternalAccess(options);
        await this.waitForInitialization(skipReady);
        try {
    ...
    async clear(options = {}) {
        ...
        try {
            ...
            return await this.clearPersistentStorage(createStorageInternalAccessOptions(options));
        } catch (error) {
...
Object.defineProperty(window, '__installStorageInternalAccess', {
    value(install) {
        ...
        return install(createStorageInternalAccessOptions, hasStorageInternalAccess);
    },
    enumerable: false,
    configurable: true,
    writable: false
