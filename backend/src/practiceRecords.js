...
function normalizeSyncTimestamp(value, fieldName) {
    const timestamp = Number(new Date(value));
    if (!Number.isFinite(timestamp) || timestamp <= 0) {
        throw requestError(`${fieldName} must be a valid timestamp`, 400, { field: fieldName });
    }
    return new Date(timestamp).toISOString();
}

function maxTimestamp(left, right) {
    return Number(new Date(left)) >= Number(new Date(right)) ? left : right;
}

function synchronizeRecords(records, incomingRecords, tombstones, clearedAt) {
    const clearTimestamp = clearedAt ? Number(new Date(clearedAt)) : 0;
    const tombstoneMap = new Map(tombstones.map((item) => [String(item.recordId), item.deletedAt]));
    const visibleExisting = normalizeRecordList(records).filter((record) => {
        const deletedAt = tombstoneMap.get(String(record.id));
        const recordTimestamp = getRecordTimestamp(record);
        return recordTimestamp > clearTimestamp && (!deletedAt || recordTimestamp > Number(new Date(deletedAt)));
    });
    const acceptedIncoming = [];
    for (const record of normalizeRecordList(incomingRecords)) {
        const recordTimestamp = getRecordTimestamp(record);
        if (recordTimestamp <= clearTimestamp) continue;
        const deletedAt = tombstoneMap.get(String(record.id));
        if (deletedAt && recordTimestamp <= Number(new Date(deletedAt))) continue;
        if (deletedAt) tombstoneMap.delete(String(record.id));
        acceptedIncoming.push(record);
    }
    return {
        records: mergePracticeRecords(visibleExisting, acceptedIncoming),
        tombstones: Array.from(tombstoneMap, ([recordId, deletedAt]) => ({ recordId, deletedAt })),
...
class PostgresPracticeRecordStore {
    ...
    async listTombstones(userId) {
        const result = await this.db.query(
            'SELECT record_id, deleted_at FROM practice_record_tombstones WHERE user_id = $1',
            [userId]
        );
        return result.rows.map((row) => ({ recordId: row.record_id, deletedAt: new Date(row.deleted_at).toISOString() }));
    }

    async replaceTombstones(userId, tombstones) {
        await this.db.withTransaction(async (client) => {
            await client.query('DELETE FROM practice_record_tombstones WHERE user_id = $1', [userId]);
            for (const tombstone of tombstones) {
                await client.query(
                    'INSERT INTO practice_record_tombstones (user_id, record_id, deleted_at) VALUES ($1, $2, $3)',
                    [userId, tombstone.recordId, tombstone.deletedAt]
                );
            }
        });
    }
    ...
    async getClearedAt(userId) {
        const result = await this.db.query(
            'SELECT cleared_at FROM practice_record_sync_state WHERE user_id = $1',
            [userId]
        );
        return result.rows[0]?.cleared_at ? new Date(result.rows[0].cleared_at).toISOString() : null;
    }

    async setClearedAt(userId, clearedAt) {
        await this.db.query(
            `INSERT INTO practice_record_sync_state (user_id, cleared_at)
             VALUES ($1, $2)
...
class MemoryPracticeRecordStore {
    constructor() {
        ...
        this.tombstonesByUser = new Map();
        this.clearedAtByUser = new Map();
    ...
    async listTombstones(userId) {
        return (this.tombstonesByUser.get(userId) || []).map((item) => ({ ...item }));
    }

    async replaceTombstones(userId, tombstones) {
        this.tombstonesByUser.set(userId, tombstones.map((item) => ({ ...item })));
    }

    async getClearedAt(userId) {
        return this.clearedAtByUser.get(userId) || null;
    }

    async setClearedAt(userId, clearedAt) {
        this.clearedAtByUser.set(userId, clearedAt);
    }
}

function createPracticeRecordService(store) {
    async function readSyncState(userId) {
        const [records, tombstones, clearedAt] = await Promise.all([
            store.list(userId),
            store.listTombstones(userId),
            store.getClearedAt(userId)
        ]);
        return { records, tombstones, clearedAt };
    }

    async function writeSyncState(userId, state) {
        await store.replace(userId, state.records);
        await store.replaceTombstones(userId, state.tombstones);
        if (state.clearedAt) await store.setClearedAt(userId, state.clearedAt);
    }

    async function withUserLock(userId, operation) {
        return store.withUserLock(userId, operation);
    }
    ...
    return {
        ...
        async replace(userId, records) {
            return withUserLock(userId, () => store.replace(userId, deduplicatePracticeRecordList(records)));
        },
        async import(userId, records) {
            return withUserLock(userId, async () => {
                const state = await readSyncState(userId);
                const next = synchronizeRecords(state.records, records, state.tombstones, state.clearedAt);
                await writeSyncState(userId, next);
                return {
                    records: next.records,
                    deletedRecordIds: next.tombstones.map((item) => item.recordId),
                    clearedAt: next.clearedAt
                };
            });
        },
        async deleteById(userId, id, deletedAt) {
            const recordId = requireRecordId(id);
            const deletionTime = normalizeSyncTimestamp(deletedAt || new Date().toISOString(), 'deletedAt');
            return withUserLock(userId, async () => {
                const state = await readSyncState(userId);
                const tombstones = new Map(state.tombstones.map((item) => [item.recordId, item.deletedAt]));
                tombstones.set(recordId, tombstones.has(recordId) ? maxTimestamp(tombstones.get(recordId), deletionTime) : deletionTime);
                const threshold = Number(new Date(tombstones.get(recordId)));
                const records = state.records.filter((record) => String(record.id) !== recordId || getRecordTimestamp(record) > threshold);
                const next = { records, tombstones: Array.from(tombstones, ([tombstoneId, deletedAt]) => ({ recordId: tombstoneId, deletedAt })), clearedAt: state.clearedAt };
                await writeSyncState(userId, next);
                return { removed: state.records.length - records.length, deletedAt: tombstones.get(recordId) };
            });
        },
        async clear(userId, clearedAt) {
            const clearTime = normalizeSyncTimestamp(clearedAt || new Date().toISOString(), 'clearedAt');
            return withUserLock(userId, async () => {
                const state = await readSyncState(userId);
                const effectiveClearAt = state.clearedAt ? maxTimestamp(state.clearedAt, clearTime) : clearTime;
                const clearTimestamp = Number(new Date(effectiveClearAt));
                const next = {
                    records: state.records.filter((record) => getRecordTimestamp(record) > clearTimestamp),
                    tombstones: state.tombstones.filter((item) => Number(new Date(item.deletedAt)) > clearTimestamp),
                    clearedAt: effectiveClearAt
                };
                await writeSyncState(userId, next);
                return { records: next.records, clearedAt: effectiveClearAt };
            });
        }
    };
}
...
function createPracticeRecordsRouter(options = {}) {
    ...
    router.post('/import', verifyCsrfToken, requireDataManageStepUp, async (req, res, next) => {
        try {
            ...
            const snapshot = await service.import(req.session.user.id, records);
    ...
    router.delete('/:id', verifyCsrfToken, requireDataManageStepUp, async (req, res, next) => {
        try {
            const result = await service.deleteById(req.session.user.id, req.params.id, req.body?.deletedAt);
    ...
    router.delete('/', verifyCsrfToken, requireDataManageStepUp, async (req, res, next) => {
        try {
            const result = await service.clear(req.session.user.id, req.body?.clearedAt);
