...
function createRateLimiter({ limit = 20, windowMs = 60_000, maxKeys = 5_000 } = {}) {
    const hits = new Map();
    return (key) => {
        const now = Date.now();
        const windowStart = now - windowMs;
        for (const [trackedKey, timestamps] of hits) {
            const active = timestamps.filter((value) => value > windowStart);
            if (active.length) hits.set(trackedKey, active);
...
class PostgresAuthStore {
    ...
    async createFirstUser({ username, usernameLower, passwordHash }) {
        return this.db.withTransaction(async (client) => {
            await client.query("SELECT pg_advisory_xact_lock(hashtext('ielts-practice:first-user'))");
...
class MemoryAuthStore {
    ...
    async createFirstUser({ username, usernameLower, passwordHash }) {
        ...
        return this.createUser({ username, usernameLower, passwordHash });
    }
}
...
function createAuthRouter(options = {}) {
    ...
    router.post('/register', verifyCsrfToken, async (req, res, next) => {
        try {
            ...
            const user = registrationMode === 'first-user'
                ? await store.createFirstUser({ username, usernameLower, passwordHash })
                : await store.createUser({ username, usernameLower, passwordHash });
            if (!user) {
