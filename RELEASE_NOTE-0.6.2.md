# IELTS Practice v0.6.2-fix Release Notes

## 🏷️ 版本概览

本版本（v0.6.2-fix）是面向 `release` 预发布的修复版，基于 v0.6.2 的架构升级继续收敛版本号、发布文档和公开仓库的资源说明。本次更新保留前端 bundle 化、练习记录统一化、阅读运行时和静态 CI 验证工作流，同时补充听力资源的版权合规说明。

### 相对 v0.6.2 的修正
* **版本号收敛**：将运行时代码、导出数据版本、开发包版本和测试夹具中的发布版本统一为 `0.6.2-fix`。
* **听力资源说明修正**：公开仓库和普通发布包默认不包含听力音频、配套题源、PDF、完整 `ListeningPractice/` 目录或预生成听力索引；相关路径仅作为可选本地扩展资源入口。
* **文档修正**：README 的听力练习、发布打包、题库资产和常见问题说明已按可选本地扩展口径更新，避免用户把合规移除误判为仓库缺失。
...
* **冗余清理**：彻底废弃并删除了过时的单点 UI 控制脚本 `js/practice-page-ui.js`，将所有散落的 UI 交互与生命周期钩子合并入统一的页面处理器中。
...
### 4. 可选听力本地扩展与 PDF 支持
* 听力练习源作为可选本地扩展接入，支持用户在个人环境中配置本地听力库，并在前端渲染关联的 PDF 资源。
* 公开仓库出于版权合规要求不随包提供听力音频、配套题源和预生成听力索引；`assets/generated/listening-exams/*` 仅作为本地生成资产的约定路径。
